...
@section('content')
    ...
    @if($flash=session('message'))
      ...
      <ol class="breadcrumb">
        ...
        <li class="active"> Nos NewsLetter</li>
      </ol>
    </section>
        ...
        <div class="col-xs-12">
          <div class="box">
            <div class="box-header">
              <h3 class="box-title"> Table des NewsLetters</h3>
            </div>
            <!-- /.box-header -->
            <div class="box-body">
              ...
              <?php use Carbon\Carbon; Carbon::setLocale('fr');
                  ...
                  @foreach($messages as $message)
                   <tr>
                     <td hidden="hidden">{{$message->id}}</td>
                        <td>{{date('d/m/Y H:i', strtotime($message->created_at))}}</td>
                  <td>{{$message->email}}</td>
                  
                       
                        <td><p ><a data-placement="top" data-toggle="tooltip"  title="Supprimer" onclick="return sure();" href="/newlettre/{{$message->id}}"class="btn btn-danger btn-xs"><i class="fa fa-trash"></i>
 ...
 function sure()
{
    return(confirm('Etes-vous sûr de vouloir supprimer ce email ?'));
}
</script>

...
@section('content')
        ...
        <div class="col-md-10">
          ...
          <div class="box box-info">
            ...
            <div class="box-body pad">
                <form method="post" action="/modifierProjet/{{$projet->id}}" enctype="multipart/form-data">
         
        {{ csrf_field() }} 
         
  <div class="form-group {{ $errors->has('nom') ? ' has-error' : '' }}">
    <label for="nom">Nom:</label>
    <input type="text"  placeholder="Enter Nom ..." class="form-control " id="nom" name="nom"  value="{{ $projet->nom }}"  required>
  @if ($errors->has('nom'))
  ...
  <div class="form-group {{ $errors->has('description') ? ' has-error' : '' }}">
    ...
    <textarea id="description"  placeholder="Enter description ..." name="description" class="textarea " style="width: 100%; height: 100px; font-size: 14px; line-height: 18px; border: 1px solid #dddddd; padding: 10px;" required >{{ $projet->description }}</textarea>
    ...
    <textarea id="appartements" placeholder="Enter les types d'appartements ..." name="appartements" class="textarea " style="width: 100%; height: 100px; font-size: 14px; line-height: 18px; border: 1px solid #dddddd; padding: 10px;" required >{{ $projet->appartements }}</textarea>
    ...
    <textarea id="partieC" placeholder="Enter les parties communes ..." name="partieC" class="textarea " style="width: 100%; height: 100px; font-size: 14px; line-height: 18px; border: 1px solid #dddddd; padding: 10px;" required >{{  $projet->partieC }}</textarea>
    ...
    <textarea id="parking" placeholder="Enter les types d'parking ..." name="parking" class="textarea " style="width: 100%; height: 100px; font-size: 14px; line-height: 18px; border: 1px solid #dddddd; padding: 10px;" required >{{ $projet->parking }}</textarea>
    ...
    <input type="url" class="form-control " id="localisation" name="localisation"  value="{{ $projet->localisation }}"  placeholder="https://example.com"
  ...
  @if ($errors->has('localisation'))
    ...
    <input type="text"  class="form-control " id="disponibilite" name="disponibilite"  value="{{ $projet->disponibilite }}"  placeholder="Disponibilité"
              >
  @if ($errors->has('disponibilite'))
   ...
   <select class="form-control select2" id="type" required  name="type"  data-placeholder="Selectoinner  Type de projet" style="width: 100%;">
    ...
    @foreach($types as $type)
     <option <?php if ($projet->type==$type->nom) {?> selected <?php } ?> >{{$type->nom}}</option>
                  ...
                  @endforeach
                        ...
                        <div lass="form-group{{ $errors->has('filename.*') ? ' has-error' : '' }}">
                       <label> Ajouter de nouveaux Photos de projet</label>
      <div class="input-group control-group increment" id="increment1" >
        ...
        <div class="clone hide" id="clone1">
          ...
          @if ($errors->has('filename.*'))
                   ...
                   <li >
                  <span class="mailbox-attachment-icon has-img"><img  src="/image/{{$photo->nom}}" id="{{$photo->nom}}" alt="Attachment"></span>

                  <div class="mailbox-attachment-info" >
                   
                        <span class="mailbox-attachment-size">
                         
                          <a href="/supprimerPhoto1/{{$photo->id}}" class="btn btn-default btn-xs pull-right"><i class=" glyphicon glyphicon-remove"></i></a>
                        </span>
                  </div>
                </li>
                  <div id="myModal" class="modal">
  <span class="close">×</span>
  <img class="modal-content" id="img01" style="height: 1000px;width:1300px " >
  <div id="caption"></div>
</div>
<script>
// Get the modal
...
var img = document.getElementById('{{$photo->nom}}');
...
span.onclick = function() { 
    ...
    <label for="type">Etat d'avancement:</label>
            ...
            <input type="number" name="pourcentage" id="pourcentage"  class="form-control "  value="{{ $projet->pourcentage  }}"/>
                      ...
                      <div lass="form-group {{ $errors->has('filename2.*') ? ' has-error' : '' }}">
                       <label> Ajouter de nouveaux Photos d'etat d'avancement</label>
          ...
          @if ($errors->has('filename2.*'))
              ...
              <ul class="mailbox-attachments clearfix">
               ...
               @foreach($projet->photos_etats as $photo2)
                <li>
                  <span class="mailbox-attachment-icon has-img myImg"><img  id="{{$photo2->nom}}" src="/image/{{$photo2->nom}}" alt="Attachment"></span>

                  <div class="mailbox-attachment-info">
                
                        <span class="mailbox-attachment-size">
                        
                         <a  href="/supprimerPhoto2/{{$photo2->id}}"class="btn btn-default btn-xs pull-right"><i class=" glyphicon glyphicon-remove"></i></a>
                        </span>
                  </div>
                </li>
                <div id="myModal" class="modal">
...
// Get the image and insert it inside the modal - use its "alt" text as a caption
var img = document.getElementById('{{$photo2->nom}}');
var modalImg = document.getElementById("img01");
var captionText = document.getElementById("caption");
img.onclick = function(){
    modal.style.display = "block";
  ...
  <div class="form-group">
   ...
   <select class="form-control select2" id=type_offre"  name="type_offre" data-placeholder="Selectoinner  Type d'offre" style="width: 100%;">
    ...
    @foreach($offres as $offre)
     <option <?php if (($projet->offre!=null)&&($projet->offre->type==$offre->nom)) {?> selected <?php } ?>>{{$offre->nom}}</option>
    ...
    <label for="date_debut">Date debut Offre:</label>
     <div class="input-group date">
               ...
               <input type="date" class="form-control pull-right"   name="date_debut" id="date_debut" <?php if ($projet->offre!=null) {?>  value="{{$projet->offre->date_debut}}" <?php } ?>/>
                               </div>
    ...
    <label for="date_fin">Date Fin Offre:</label>
     <div class="input-group date">
               ...
               <input type="date" class="form-control pull-right"   name="date_fin" id="date_fin" <?php if ($projet->offre!=null) {?>  value="{{$projet->offre->date_fin}}" <?php } ?> />
                </div>

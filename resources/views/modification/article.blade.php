...
@section('content')
        ...
        <div class="col-md-10">
          ...
          <div class="box box-info">
            ...
            <div class="box-body pad">
              <form method="post"  action="/modifierArticle/{{$article->id}}" enctype="multipart/form-data">
                 {{ csrf_field() }} 

  <div class="form-group{{ $errors->has('titre') ? 'has-error' : '' }}">
    <label for="titre">Titre:</label>
    <input type="text" placeholder="Titre..."  class="form-control" id="titre" name="titre"  value="@if (isset($article)) {{$article->titre}} @endif"  required>
  @if ($errors->has('titre'))
  ...
  <div class="form-group{{ $errors->has('resume') ? 'has-error' : '' }}">
    ...
    <textarea id="resume" placeholder="Résumé..."  name="resume" class="form-control" required >@if (isset($article)) {{$article->resume}} @endif</textarea>
  ...
  <textarea class="textarea {{ $errors->has('text') ? 'has-error' : '' }}" placeholder="Enter le text de l'article" id="text" name="text"
                          style="width: 100%; height: 200px; font-size: 14px; line-height: 18px; border: 1px solid #dddddd; padding: 10px;" required >@if (isset($article)) {{$article->text}} @endif</textarea>
                           @if ($errors->has('text'))
                                    <span class="help-block">
   ...
   <select class="form-control select2" id="mot_cles[]"  name="mot_cles[]" multiple="multiple" data-placeholder="Selectoinner  mot clé(s)" style="width: 100%;">
    ...
    <label for="photo">Photo:</label>
         <input type="file" name="photo" id="photo" class="form-control" name="photo"   >
           @if ($errors->has('photo'))
                                    <span class="help-block">
...
<div class="form-group{{ $errors->has('video') ? ' has-error' : '' }}">
    ...
    <input type="url" placeholder="Source"class="form-control" id="source" name="source"  value="@if (isset($article)) {{$article->source}} @endif"  placeholder="https://example.com"
               pattern="https://.*" >

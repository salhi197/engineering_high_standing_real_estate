...
@section('content')
    ...
    <section class="sidebar">
      ...
      <ul class="sidebar-menu" data-widget="tree">
        ...
        <li class="active treeview">
          ...
          <ul class="treeview-menu">
            <li <?php if($slider->type=='projet') {?>class="active" <?php } ?>><a href="/slider/projets"><i class="fa fa-angle-right"></i> Slider des Projets</a></li>
            <li <?php if($slider->type=='article') {?>class="active" <?php } ?>><a href="/slider/articles"><i class="fa fa-angle-right"></i> Slider des Articles</a></li>
    ...
    <section class="content-header">
      ...
      <ol class="breadcrumb">
        ...
        <li class="active">Modification Slider</li>
      </ol>
    </section>
        ...
        <div class="col-md-8">
          ...
          Information du Slider
            ...
            <div class="box-body pad">
             <form method="post" action="/modificationSlider/{{$slider->id}}"  enctype="multipart/form-data" >
         
        {{ csrf_field() }} 
        <input type="text" name="type" id="text" value="{{$slider->type}}" hidden="hidden">
 
        <div class="form-group{{ $errors->has('titre') ? 'has-error' : '' }}">
    <label for="titre">Titre:</label>
    <input type="text" placeholder="Titre..."  class="form-control" id="titre" name="titre"  value="{{ $slider->titre }}"  required>
  @if ($errors->has('titre'))
 ...
 <div class="form-group{{ $errors->has('text') ? 'has-error' : '' }}">
  <textarea class="textarea " placeholder="Enter le text de l'article" id="text" name="text"
                          style="width: 100%; height: 200px; font-size: 14px; line-height: 18px; border: 1px solid #dddddd; padding: 10px;" required >{{ $slider->text }}</textarea>
                       @if ($errors->has('text'))
                                    <span class="help-block">
...
<!-- @if($slider->type=='projet')
 <div class="form-group">
                <label>Projet</label>
                <select class="form-control" id="projet" style="width: 100%;" name="projet_id">
                  @foreach($projets as $projet)
                  <option <?php if($slider->projet_id==$projet->id) {?> selected <?php } ?> value="{{$projet->id}}">{{$projet->nom}}</option>
                 @endforeach
                </select>
              </div>
@else
 <div class="form-group">
                ...
                <select class="form-control" id="projet" style="width: 100%;" name="article_id">
                  ...
                  <option <?php if($slider->article_id==$article->id) {?> selected <?php } ?> value="{{$article->id}}">{{$article->titre}}</option>
                 @endforeach
                </select>
              </div>
@endif
